...
class Admin extends Base
{
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $result = $adminModel->addData($data);
            if ($result) {
                $adata['uid'] = $adminModel->uid;
                $adata['group_id'] = $group_id;
                (new \app\common\model\AuthGroupAccess())->addData($adata);
                $return['uid'] = $adminModel->uid;
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            $data['email'] = $email;
            $adminModel = new \app\common\model\Admin();
            $result = $adminModel->saveData($map, $data);
            $adata['uid'] = $id;
            $adata['group_id'] = $group_id;
            (new \app\common\model\AuthGroupAccess())->saveData($adata, ["uid" => $id]);
            if ($result) {
                $return['uid'] = $id;
